...
function AcslAgentSalesInner() {
  ...
  const fetchAllSalesForSummary = useCallback(async () => {
    ...
    try {
      const filters: Record<string, any> = { limit: 1000, responseFormat: "format2" };
  ...
  useEffect(() => { fetchAllSalesForSummary(); }, [fetchAllSalesForSummary]);
  useEffect(() => { setCurrentPage(1); }, [searchTerm, orgFilter, approvalFilter]);
  ...
  const hasActiveFilters = searchTerm !== "" || orgFilter !== "all" || approvalFilter !== "all";

  const handleApproveSuccess = (saleId: string) => {
    setSales((prev) =>
      prev.map((s) =>
        s.id === saleId ? { ...s, agent_approved: true, agent_approved_at: new Date().toISOString() } : s
      )
    );
    setAllSalesForSummary((prev) =>
      prev.map((s) => s.id === saleId ? { ...s, agent_approved: true } : s)
    );
  ...
  const financialSummary = useMemo(() => {
    const getAmountPaid = (s: Sale) => s.is_installment ? (s.total_paid ?? 0) : (s.amount ?? 0);
  ...
  const toggleBtn = (show: boolean, onToggle: () => void) => (
    <Button variant="ghost" size="sm"
      ...
      onClick={onToggle}
    >
      {show ? <><p>Hide</p><EyeOff className="h-4 w-4" /></> : <><p>Show</p><Eye className="h-4 w-4" /></>}
    </Button>
  );
  ...
  return (
    <DashboardLayout currentRoute="sales" title="Sales">
      <div className="p-6 space-y-4">
        ...
        {!summaryLoading && (
          <div>
            ...
            {showStatusBreakdown && (
              <PaymentStatusCards breakdown={paymentBreakdown} activeFilter={paymentStatusFilter} onFilterClick={handleCardFilterClick} />
            )}
          </div>
        )}
        ...
        <div className="bg-blue-50 p-3 rounded-lg border border-gray-200">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <Input placeholder="Search customer, phone, serial no..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 bg-white" />
            </div>

            <div className="flex-1 min-w-[180px]">
              <Select value={orgFilter} onValueChange={setOrgFilter}>
                <SelectTrigger className="bg-white"><SelectValue placeholder="All Partners" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="flex-1 min-w-[150px]">
              <Select value={approvalFilter} onValueChange={setApprovalFilter}>
                <SelectTrigger className="bg-white"><SelectValue placeholder="All Approvals" /></SelectTrigger>
                <SelectContent>
        ...
        <div className="space-y-0">
                ...
                {!loading && sales.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10} className="text-center py-12 text-gray-500">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p>No sales found.</p>
                        {hasActiveFilters && <p className="text-xs">Try clearing your filters.</p>}
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
                  sales.map((sale, index) => (
                    <TableRow key={sale.id} className={`${index % 2 === 0 ? "bg-white" : "bg-blue-50/50"} hover:bg-gray-50 text-gray-700`}>
                      <TableCell className="font-medium text-gray-900 text-sm">{sale.contact_person || sale.end_user_name || "—"}</TableCell>
                      <TableCell className="text-gray-600 text-sm">{sale.partner_name || "—"}</TableCell>
                      <TableCell className="font-mono text-gray-600 text-sm">{sale.stove_serial_no || "—"}</TableCell>
                      <TableCell className="text-gray-600 text-sm">{sale.amount != null ? `₦${sale.amount.toLocaleString()}` : "—"}</TableCell>
                      <TableCell className="text-gray-600 text-sm">{sale.state_backup || "—"}</TableCell>
                      <TableCell>
                        <Badge className={sale.status === "completed" ? "bg-green-100 text-green-800 border-green-200" : sale.status === "pending" ? "bg-yellow-100 text-yellow-800 border-yellow-200" : "bg-gray-100 text-gray-800 border-gray-200"}>
                          {sale.status || "—"}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {sale.is_installment ? (
                          sale.payment_status === "fully_paid" ? (
                            <Badge className="bg-green-100 text-green-800 border-green-200 text-xs">Fully Paid</Badge>
                          ) : sale.payment_status === "partially_paid" ? (
                            <div className="flex flex-col gap-0.5">
                              <Badge className="bg-amber-100 text-amber-800 border-amber-200 text-xs">Partially Paid</Badge>
                              <span className="text-xs text-gray-500">₦{(sale.total_paid ?? 0).toLocaleString()} / ₦{(sale.amount ?? 0).toLocaleString()}</span>
                            </div>
                          ) : (
                            <Badge className="bg-blue-100 text-blue-800 border-blue-200 text-xs">Installment</Badge>
                          )
                        ) : (
                      ...
                      <TableCell>
                        {sale.agent_approved ? (
                          <Badge className="bg-green-100 text-green-800 border-green-200"><CheckCircle className="h-3 w-3 mr-1" />Approved</Badge>
                        ) : (
                          <Badge className="bg-orange-100 text-orange-800 border-orange-200"><Clock className="h-3 w-3 mr-1" />Pending</Badge>
                        )}
                      </TableCell>
                      ...
                      <TableCell className="text-center">
                        <Button
                          ...
                          className={sale.agent_approved ? "bg-gray-100 text-gray-400 cursor-not-allowed text-xs" : "bg-green-600 hover:bg-green-700 text-white text-xs"}
                          onClick={() => { if (!sale.agent_approved) { setSelectedSale(sale); setShowApproveModal(true); } }}
                        >
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className="border border-t-0 border-gray-200 rounded-b-lg px-4 py-3 flex items-center justify-between bg-white">
              <p className="text-sm text-gray-600">Showing {startItem} to {endItem} of {pagination.totalItems} sales</p>
              <div className="flex items-center gap-1">
                ...
                <Button variant="outline" size="sm" className="h-8 px-2" onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1}><ChevronLeft className="h-4 w-4 mr-1" />Prev</Button>
                {getPageNumbers().map((p) => (
                  <Button key={p} variant={p === currentPage ? "default" : "outline"} size="sm" className={`h-8 w-8 p-0 ${p === currentPage ? "bg-brand text-white hover:bg-brand" : ""}`} onClick={() => setCurrentPage(p)}>{p}</Button>
                ))}
                <Button variant="outline" size="sm" className="h-8 px-2" onClick={() => setCurrentPage((p) => Math.min(pagination.totalPages, p + 1))} disabled={currentPage >= pagination.totalPages}>Next<ChevronRight className="h-4 w-4 ml-1" /></Button>
                <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => setCurrentPage(pagination.totalPages)} disabled={currentPage >= pagination.totalPages}><ChevronsRight className="h-4 w-4" /></Button>
              </div>
            </div>
          )}
        </div>
      ...
      {showApproveModal && selectedSale && (
        <ApproveSaleConfirmModal sale={selectedSale} onClose={() => { setShowApproveModal(false); setSelectedSale(null); }} onSuccess={handleApproveSuccess} />
      )}
      {showCreateModal && <AgentCreateSaleModal onClose={() => setShowCreateModal(false)} />}
      <ToastContainer toasts={toasts} onRemove={removeToast} />
    </DashboardLayout>
  );
}

export default function AcslAgentSalesContent() {
  return (
