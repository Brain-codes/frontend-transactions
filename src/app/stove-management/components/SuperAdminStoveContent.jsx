...
export default function SuperAdminStoveContent() {
  ...
  const fetchStoveIds = async (page = 1, pageSize = pagination.page_size, currentFilters = filters, orgIds = selectedOrgIds, archived = showArchived, currentSortBy = sortBy, currentSortDir = sortDir) => {
    ...
    try {
      ...
      toast({ variant: "error", title: "Failed to fetch stove IDs", description: err.message });
      setStoveIds([]);
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      if (stoveData.status === "sold") {
        const saleResponse = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-sale?stove_serial_no=${stoveData.stove_id}`,
          { headers: { Authorization: `Bearer ${session.access_token}` } }
        );
