...
import {
  Loader2, Search, X, Package, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
} from "lucide-react";
...
export default function AcslAgentStoveContent() {
  ...
  const fetchStoveIds = useCallback(
    async (page = 1) => {
      if (assignedOrgIds.length === 0) { setStoveIds([]); setLoading(false); return; }
      setLoading(true);
      setError(null);
      try {
        ...
        setPagination(result.pagination || { page: 1, page_size: PAGE_SIZE, total_count: 0, total_pages: 0 });
      } catch (err: any) {
  ...
  useEffect(() => {
    if (assignedOrgIds.length > 0) fetchStoveIds(1);
    else if (!loading && assignedOrgIds.length === 0 && user?.id) setLoading(false);
  }, [assignedOrgIds, orgFilter, searchTerm, statusFilter]);

  const hasActiveFilters = searchTerm !== "" || statusFilter !== "all" || orgFilter !== "all";

  const handleClearFilters = () => { setSearchTerm(""); setStatusFilter("all"); setOrgFilter("all"); };

  const getStatusBadge = (status: string) => {
    if (status === "sold") return <Badge className="bg-red-100 text-red-800 border-red-200">Sold</Badge>;
  ...
  return (
    <DashboardLayout currentRoute="stove-management" title="Stove IDs">
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-amber-100 rounded-lg"><Package className="h-5 w-5 text-amber-700" /></div>
              <div>
        ...
        <div className="bg-blue-50 p-3 rounded-lg border border-gray-200">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex-1 min-w-[200px] relative">
              ...
              <Input placeholder="Search stove ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 bg-white" />
            </div>
              ...
              <TableBody>
                {!loading && stoveIds.length === 0 ? (
                  ...
                  stoveIds.map((stove, index) => (
                    <TableRow key={stove.stove_id} className={`${index % 2 === 0 ? "bg-white" : "bg-blue-50/50"} hover:bg-gray-50 text-gray-700`}>
                      ...
                      <TableCell className="text-gray-600 text-sm">{stove.organization_name || "—"}</TableCell>
                      <TableCell className="text-gray-600 text-sm">{stove.branch || "—"}</TableCell>
                      <TableCell className="text-gray-600 text-sm">{formatDate(stove.date_sold)}</TableCell>
                      <TableCell className="text-gray-600 text-sm">{stove.sold_to || "—"}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
          ...
          {pagination.total_pages > 1 && (
            <div className="border border-t-0 border-gray-200 rounded-b-lg px-4 py-3 flex items-center justify-between bg-white">
              ...
              <div className="flex items-center gap-1">
                <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => fetchStoveIds(1)} disabled={pagination.page === 1}><ChevronsLeft className="h-4 w-4" /></Button>
                <Button variant="outline" size="sm" className="h-8 px-2" onClick={() => fetchStoveIds(Math.max(1, pagination.page - 1))} disabled={pagination.page === 1}><ChevronLeft className="h-4 w-4 mr-1" />Prev</Button>
                ...
                <Button variant="outline" size="sm" className="h-8 px-2" onClick={() => fetchStoveIds(Math.min(pagination.total_pages, pagination.page + 1))} disabled={pagination.page === pagination.total_pages}>Next<ChevronRight className="h-4 w-4 ml-1" /></Button>
                <Button variant="outline" size="sm" className="h-8 w-8 p-0" onClick={() => fetchStoveIds(pagination.total_pages)} disabled={pagination.page === pagination.total_pages}><ChevronsRight className="h-4 w-4" /></Button>
              </div>
            </div>
          )}
        </div>
