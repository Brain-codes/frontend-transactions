...
interface FinancialReportsViewProps {
  /** Async function that resolves to { success, data, error } */
...
const FinancialReportsView: React.FC<FinancialReportsViewProps> = ({ loadSales }) => {
  ...
  const filteredSales = useMemo(() => {
    ...
    if (startDate) {
      result = result.filter((s) => (s.sales_date || s.created_at) >= startDate);
    }
    if (endDate) {
      result = result.filter((s) => (s.sales_date || s.created_at) <= endDate + "T23:59:59");
    }

    result.sort((a, b) => {
      const dateA = new Date(a.sales_date || a.created_at).getTime();
      const dateB = new Date(b.sales_date || b.created_at).getTime();
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, paymentStatusFilter, startDate, endDate, pageSize]);

  const hasActiveFilters =
    searchTerm !== "" || paymentStatusFilter !== "all" || startDate !== "" || endDate !== "";
  ...
  return (
    <div className="space-y-4 px-4 py-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold" style={{ color: "rgb(7, 55, 106)" }}>
        ...
        <>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="ghost" size="sm" className="h-7 w-fit p-0 flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 transition-colors" onClick={() => setShowFinancialSummary(!showFinancialSummary)}>
                {showFinancialSummary ? <div className="flex gap-2 w-fit"> <p>Hide </p> <EyeOff className="h-4 w-4" /></div> :  <div className="flex gap-2 w-fit"> <p>Show </p> <Eye className="h-4 w-4 " /></div>}
              </Button>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="ghost" size="sm" className="h-7 p-0 flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-800 transition-colors" onClick={() => setShowStatusBreakdown(!showStatusBreakdown)}>
                {showStatusBreakdown ?<div className="flex gap-2 w-fit"> <p>Hide </p> <EyeOff className="h-4 w-4" /></div> :  <div className="flex gap-2 w-fit"> <p>Show </p> <Eye className="h-4 w-4 " /></div>}
              </Button>
            </div>
            {showStatusBreakdown && <PaymentStatusCards breakdown={paymentBreakdown} />}
          </div>
