...
const SuperAdminDashboardContent = () => {
  ...
  useEffect(() => {
    ...
    if (cached && cached.length > 0) setOrganizations(cached);
  }, []);
  ...
  const fetchOrganizationsForSearch = useCallback(
    async (searchTerm = "") => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(result.error || "Failed to fetch organizations");
  ...
  const getSelectedOrgName = () => {
    ...
    if (selectedOrgIds.length === group.branches?.length || 0) return group.base_name;
  ...
  const fetchDashboardStats = async (appliedFilters = filters) => {
    ...
    try {
      ...
      const response = await superAdminDashboardService.getDashboardStats(filterParams);
      if (response.success) {
  ...
  const handleClearFilters = () => {
    const clearedFilters = { date_from: "", date_to: "", customer_state: "", period: "6months" };
  ...
  const hasActiveFilters =
    Object.values(filters).some((value) => value !== "" && value !== "6months") ||
  ...
  const stoveStatus = (dashboardData?.stoveStatus || []).filter((s) => s.status !== "Received");
            ...
            <div className="flex flex-wrap items-center gap-4">
                            ...
                            organizations.map((group) => (
                              <div key={group.base_name}>
                                <div
                                  ...
                                  onClick={() => {
                                    handleSelectOrganization(group.organization_ids);
                                    setOrgSearch("");
                                  }}
                                >
                                  ...
                                  group.branches.map((branch) => (
                                    ...
                                    >
                                      <div className="flex items-center justify-between w-full">
                                        ...
                                        {branch.state && (
                                          <span className="text-xs text-gray-500">{branch.state}</span>
                                        )}
                                      </div>
                                    </div>
                                  ))}
              ...
              {selectedOrgIds.length > 0 && (
                <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-md border border-gray-200">
                  ...
                  <span className="text-sm font-medium text-gray-900">{getSelectedOrgName()}</span>
                  <button
                    onClick={() => { handleSelectOrganization([]); setOrgSearch(""); }}
              ...
              <div className="flex-1 min-w-[150px]">
                <Select
                  ...
                  onValueChange={(value) => handleFilterChange("customer_state", value || "")}
                >
                  <SelectTrigger className="bg-white">
                ...
                {loadingKPIs ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-600">Total Stoves Sold</p>
                      <h3 className="text-3xl font-bold text-gray-900 mt-2">{formatNumber(kpis.soldStoves)}</h3>
                      <div className="flex items-center mt-2">
                        {kpis.salesGrowth >= 0 ? (
                          <>
                            <ArrowUpRight className="h-4 w-4 text-green-600 mr-1" />
                            <span className="text-sm font-medium text-green-600">+{kpis.salesGrowth?.toFixed(1)}%</span>
                          </>
                        ) : (
                          <>
                            <ArrowDownRight className="h-4 w-4 text-red-600 mr-1" />
                            <span className="text-sm font-medium text-red-600">{kpis.salesGrowth?.toFixed(1)}%</span>
                          </>
                        )}
            ...
            <Card className="border-l-4 border-l-green-500">
              <CardContent className="p-6">
                {loadingKPIs ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-600">Stove in Stock</p>
                      <h3 className="text-3xl font-bold text-gray-900 mt-2">{formatNumber(kpis.availableStoves)}</h3>
                      <p className="text-xs text-gray-500 mt-2">Available stoves</p>
                    </div>
                ...
                {loadingKPIs ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <h3 className="text-3xl font-bold text-gray-900 mt-2">{formatNumber(kpis.totalOrganizations)}</h3>
                ...
                {loadingKPIs ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <h3 className="text-3xl font-bold text-gray-900 mt-2">{formatCurrency(kpis.totalRevenue)}</h3>
                      <div className="flex items-center mt-2">
                        {kpis.revenueGrowth >= 0 ? (
                          <>
                            <ArrowUpRight className="h-4 w-4 text-green-600 mr-1" />
                            <span className="text-sm font-medium text-green-600">+{kpis.revenueGrowth?.toFixed(1)}%</span>
                          </>
                        ) : (
                          <>
                            <ArrowDownRight className="h-4 w-4 text-red-600 mr-1" />
                            <span className="text-sm font-medium text-red-600">{kpis.revenueGrowth?.toFixed(1)}%</span>
                          </>
                        )}
                        <span className="text-xs text-gray-500 ml-2">vs previous period</span>
                      </div>
                ...
                {loadingCharts ? (
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={monthlyTrends} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <Line yAxisId="left" type="monotone" dataKey="sales" stroke={COLORS.blue} strokeWidth={3} name="Sales" dot={{ r: 4, fill: COLORS.blue }} activeDot={{ r: 6 }} />
                      <Line yAxisId="right" type="monotone" dataKey="revenue" stroke={COLORS.success} strokeWidth={3} name="Revenue" dot={{ r: 4, fill: COLORS.success }} activeDot={{ r: 6 }} />
                    </LineChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-6 mt-6 pt-4 border-t border-gray-100">
                      <div className="text-center">
                        ...
                        <p className="text-xl font-bold text-green-600">{formatNumber(kpis.availableStoves)}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-xs text-gray-500 mb-1">Sold</p>
                        <p className="text-xl font-bold text-blue-600">{formatNumber(kpis.soldStoves)}</p>
                      </div>
                    </div>
                  </>
                )}
              ...
              <CardContent>
                {loadingCharts ? (
                  ...
                  <ResponsiveContainer width="100%" height={350}>
                    <BarChart data={topOrganizations} layout="horizontal" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                      ...
                      />
                      <Legend wrapperStyle={{ paddingTop: "10px" }} iconType="circle" />
                      <Bar dataKey="sales" fill={COLORS.blue} name="Sales" radius={[0, 4, 4, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                {loadingCharts ? (
                  ...
                  <ResponsiveContainer width="100%" height={350}>
                    <BarChart data={salesByState} margin={{ top: 5, right: 30, left: 20, bottom: 80 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                      <XAxis dataKey="state" tick={{ fontSize: 11, fill: "#6b7280" }} angle={-45} textAnchor="end" height={80} stroke="#e5e7eb" />
                      ...
                      />
                      <Legend wrapperStyle={{ paddingTop: "10px" }} iconType="circle" />
                      <Bar dataKey="sales" fill={COLORS.purple} name="Sales" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
